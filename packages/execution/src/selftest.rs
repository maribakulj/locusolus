...
pub const SUITE: [Probe; 16] = [
    ...
    Probe {
        ...
        dimension: Dimension::ResourceQuota,
        contained_from: SandboxLevel::S2,
        rationale: "§32.3 : quota disque vérifié par self-test",
        critical: true,
    },
    Probe {
...
/// Ce que la sonde doit produire au niveau demandé.
#[must_use]
pub const fn expectation(probe: &Probe, level: SandboxLevel) -> Expectation {
...
#[must_use]
pub fn judge(probe: &Probe, level: SandboxLevel, observed: Observed) -> Verdict {
    match (expectation(probe, level), observed) {
...
/// une suite passée.
#[must_use]
pub fn standing(level: SandboxLevel, results: &[(&'static str, Observed)]) -> Standing {
    ...
    for probe in &SUITE {
        ...
        let verdict = judge(probe, level, observed);
