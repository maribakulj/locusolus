...
| W20.k `[R]` | la **surface §15.2** de `locusd` : par où un worker réclame, remonte et rend | découvert en marquant `W2.21` — **aucun item de cette roadmap ne servait §15.2 côté `locusd`**, et `apps/locusd/src/http.rs` le confirme : sept routes, toutes en lecture, aucune sous `/lep/`. Trois lignes pointaient déjà ce trou sans le nommer — `W23.b` a fait porter son marqueur à `W2.20` puis à `W2.21`, `W12.d` ne listait ni l'un ni l'autre. C'est le **cinquième** maillon de la fermeture verticale, et il était invisible pour la même raison que les quatre autres : chaque côté est cohérent séparément. `W2.21` sait parler les trois chemins ; personne ne les écoute. Test de sortie : `/lep/v1/claim`, `/lep/v1/events` et `/lep/v1/result` sont servis, et le harnais de conformance de `W0.9` tourne contre le **daemon réel** au lieu d'un `fetch` d'épreuve ; une réclamation sans mission assignable rend `204` et **non** une erreur — la séparation « rien pour toi » / « je n'ai pas pu demander » de l'ADR 0028 décision 4 est tenue des **deux** côtés du fil, et un test la vérifie depuis le client de `W2.21` sans le modifier ; un résultat rendu passe par un command handler transactionnel, donc l'achèvement d'une tentative **atteint le journal** — c'est le fait exact que `W23.b` compte, et rien d'autre dans cette roadmap ne le fait exister ; deux remises portant la même `idempotency_key` de §15.5 produisent **un** fait, la durabilité du registre restant l'affaire de `W20.j` pour que les deux ne se confondent pas ; une créance de worker invalide reçoit un refus **typé** de §22.5, jamais une trace ; la règle 4 de `boundaries.json` reste tenue — servir un worker n'ouvre aucun socket de runtime |
...
`W20.h` et `W20.i` sont les deux premiers des **cinq maillons de la fermeture verticale** : sans
...
| W23.b `[R]` **bloqué** | les trois compteurs : `nominal`, `active`, `generating` | locusolus | `attend:W20.k` — dépendance technique nommée, et **c'est la troisième nomination de la même condition** : le marqueur a d'abord attendu `W2.20`, puis `W2.21`, deux jalons voisins qui ont été livrés sans rien changer à ce qui manquait. La condition réelle n'a jamais bougé — le fait doit **atteindre le journal** — et ce qui l'en empêche est que `locusd` ne sert pas §15.2 : la boucle du worker réclame, remonte et rend dans le vide, personne n'écoute. Le troisième essai nomme donc un item qui n'existait pas, et c'est pour cela que les deux premiers ont visé à côté : `W20.k` a été créé en marquant `W2.21`. Un marqueur ne se périme correctement que s'il nomme la condition et non un jalon voisin — la limite de `W0.16` rencontrée deux fois de l'intérieur. `generating` compte donc un fait qu'aucun journal n'écrit, les six `InstanceState` de §7.1 n'en portant pas l'équivalent, et **inventer ce fait pour avoir quoi compter** serait ce que `W21.g` a refusé sous ce nom. Test de sortie : les trois se recalculent depuis le journal et se rejouent à l'identique ; un rapport qui n'en porte qu'un **n'est pas constructible**, tenu par le type ; `generating ≤ active ≤ nominal` est un invariant testé, et une violation nomme les trois valeurs ; aucun de ces compteurs ne porte de seuil, comme les treize métriques de `W21` |
