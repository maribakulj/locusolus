...
| W12.d `[R]` | `e2e/minimal_science` et son jumeau avec panne — **le centre de gravité du projet** | **la liste de la quatrième reprise est épuisée** — `W20.p`, `W20.q` et `W20.r` sont livrés, et avec eux les huit qui les précédaient (`W20.h`, `W20.i`, `W2.20`, `W20.k`, `W20.m`, `W20.n`, `W20.o`, et `W20.j` reste dû mais ne bloque pas ce test). **Aucun item nommé ne bloque plus celui-ci**, et c'est pourquoi cette ligne ne déclare rien : elle entre dans la frontière, comme n'importe quel item à faire. Ce qui n'est **pas** affirmé, et qui compte plus : que ce test passera. Quatre marqueurs successifs ont annoncé un déblocage qui n'a pas eu lieu, chaque fois parce que la liste sous-estimait la précondition — `W12.d` exerce la chaîne entière, donc sa précondition est « le reste de la V1 » et non une poignée d'items. La cinquième réponse n'est donc plus une liste : c'est de **tenter le test**, et de laisser ce qu'il révèle devenir des items. Les deux découvertes de `W20.q` et `W20.r` — une file qui ne plaçait rien, un commit qui mettait une projection en quarantaine — n'ont été trouvées d'aucune autre façon. *(marqueur levé)* — **la liste de la quatrième reprise est épuisée** : `W20.p`, `W20.q` et `W20.r` sont livrés, et avec eux les huit qui précédaient, et il faut en tirer la conclusion structurelle plutôt que la corriger une fois de plus à l'identique. Il a successivement attendu `W20.h`, `W20.i`, `W2.20`, `W20.k`, puis `W20.m`, `W20.n` et `W20.o` ; **les huit sont livrés** et ce test reste impossible. Une liste qui se périme quatre fois ne se trompe pas de contenu, elle se trompe de **forme** : `W12.d` exerce la chaîne entière, donc sa précondition est « le reste de la V1 » et non une poignée d'items, et tout marqueur court la sous-estimera encore.
