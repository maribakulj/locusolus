...
| W12.d `[R]` **bloqué** | `e2e/minimal_science` et son jumeau avec panne — **le centre de gravité du projet** | `attend:W20.p attend:W20.q attend:W20.r` — **quatrième reprise de ce marqueur**, et il faut en tirer la conclusion structurelle plutôt que la corriger une fois de plus à l'identique. Il a successivement attendu `W20.h`, `W20.i`, `W2.20`, `W20.k`, puis `W20.m`, `W20.n` et `W20.o` ; **les huit sont livrés** et ce test reste impossible. Une liste qui se périme quatre fois ne se trompe pas de contenu, elle se trompe de **forme** : `W12.d` exerce la chaîne entière, donc sa précondition est « le reste de la V1 » et non une poignée d'items, et tout marqueur court la sous-estimera encore.

Ce qui change ici est la méthode : chaque entrée de la nouvelle liste a été **vérifiée au code** avant d'être écrite, et non déduite du test de sortie. `MemoryQueue::take` ignore le `worker_id` — sa documentation le dit — donc « placé sur ce qu'il a prouvé » n'a pas de sujet (`W20.q`) ; rien dans `apps/locusd` ne connaît `EpistemicCommit` hors d'une traduction de type, donc « mis en scène puis intégré » n'en a pas non plus (`W20.r`) ; `W20.p` reste dû. Les phases `W5` et `W7`, elles, sont **entièrement livrées** — vérifié, pas supposé.

**La complétude de cette liste n'est pas affirmée.** C'est la seule chose que quatre expirations autorisent à dire : ce qui y figure manque réellement, et il peut en manquer d'autres que personne n'a encore cherchées. Confronté clause par clause à son propre test de sortie, il manque trois choses, et aucune n'avait d'item : **une question produit une mission** — rien ne crée de `MissionEnvelope`, la file de `W20.k` n'est remplie que par un test (`W20.o`) ; **un worker s'enregistre** — l'enrôlement de §7.2 n'est servi par personne, le registre de `W20.k` n'est rempli que par un test (`W20.n`) ; **`locusd` redémarre et tout est encore là** — `W20.i` a livré le driver sans le câbler, ce qui était son périmètre (`W20.m`). La leçon est celle de `W0.16`, apprise pour la troisième fois : un marqueur qui nomme un jalon au lieu d'une condition se périme en annonçant un déblocage qui n'a pas lieu. Test de sortie : une question produit une mission ; un worker s'enregistre et est placé sur ce qu'il a **prouvé** ; la session tourne ; les outils s'exécutent dans le confinement promis ; les événements repartent ; les artefacts sont hashés ; un `EpistemicCommit` est mis en scène puis intégré ; les projections se mettent à jour ; le graphe rend la conclusion, ses prémisses, son expérience, ses artefacts, ses objections et son coût ; `locusd` redémarre et **tout est encore là**. Le jumeau tue le worker au milieu : la lease est revalidée, la reprise se fait sous le même numéro de tentative, **zéro perte et zéro doublon**. Le confinement est celui qu'un runner peut tenir — `S1`/`S2` —, et il n'est pas **attesté** : c'est `W12.e` |
