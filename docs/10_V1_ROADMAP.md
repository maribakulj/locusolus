...
| W5.af `[R]` | une campagne qui exerce le mécanisme **du worker** | ADR 0035 décision 4, voie courte. `Proven` ne peut être rempli pour un worker réel que par une campagne exerçant le mécanisme que ce worker emploie — `bubblewrap` là où `canterel` tourne. Elle s'exécute **là où le worker est**, ce qui règle en passant une bonne part de la question d'adressage laissée ouverte par `W5.ac` : une campagne qui tourne sur la machine du worker peut connaître son identité. La voie longue — une variante d'exécution dans `Ask`, qui ferait exécuter une mission *par* le broker — est un changement de rôle du broker et demande son propre ADR |
...
| W12.f `[R]` **fait** | le **harnais** de `e2e/minimal_science` : où il vit, et qui joue le worker | il n'existe pas, et la première tentative de `W12.d` l'a établi avant d'écrire une ligne : `e2e/minimal_science` n'est nommé **nulle part** hors de cette roadmap — pas de répertoire, pas de job de CI, pas d'ADR. Et le seul worker du chantier est dans un autre dépôt, dans un autre langage ; `packages/testing` ne peut pas le remplacer, puisqu'il joue le **serveur** par construction (`W0.9`, et `W20.k` s'y est heurté). Ce que cet item tranche : où le test vit, et comment le worker réel y arrive. Les deux options ont un coût réel et opposé — faire entrer `canterel` dans la CI de `locusolus` (une chaîne Bun et un checkout épinglé, coût ponctuel) ou faire entrer `locusolus` dans celle de `canterel` (une chaîne Rust et un checkout, hors du périmètre local, donc **payé à chaque synchronisation amont**, ADR 0010). La décision demande un ADR. Elle n'est **pas** prise ici : la prendre avant que `W20.s`, `W20.t`, `W20.u` et `W2.22` existent serait décider avant l'évidence, ce que ce dépôt refuse ailleurs sous le nom d'abstraction spéculative. Test de sortie : un ADR tranche la question en mesurant les deux coûts plutôt qu'en les estimant ; le harnais démarre un `locusd`, un `locus-execd` et le worker réel, et les arrête ; il échoue **bruyamment** quand l'un des trois ne démarre pas, au lieu de rendre vert un test qui n'a rien exercé — c'est la règle du dépôt, et `W20.i` a montré ce qu'elle coûte quand on l'oublie |
| W12.g `[R]` **fait** | l'ADR 0033 décidait une révision **épinglée** ; le workflow n'avait aucun `ref:` | vérifié au fichier : l'ADR 0033 dit « `canterel` y est cloné à une **révision épinglée** » deux fois — titre de la décision 1, et conséquences, où il ajoute « **comme le SDK** » —, et l'étape « Le dépôt worker » de `ci.yml` n'avait pas de `ref:`. Le commentaire juste au-dessus affirmait pourtant « le worker réel, à une révision épinglée » : une décision écrite et non appliquée, sous une phrase qui disait le contraire de ce que le YAML faisait. Conséquence : le verdict e2e de `locusolus` dépendait silencieusement de ce que `canterel/main` se trouvait être, et un merge là-bas pouvait rendre cette CI rouge **sans aucun changement ici** — le « verdict qui peut rougir pour une raison étrangère cesse d'être lu » de la décision 3 du même ADR, appliqué dans l'autre sens. **L'objection est traitée, pas tue** : épingler fait cesser d'exercer le worker courant, et c'est son exécution qui a trouvé cinq défauts en une session. La réponse est le mot que l'ADR emploie — « comme le SDK », que `canterel` épingle par commit **et bumpe délibérément** : un pin n'est pas « ne jamais mettre à jour » mais « mettre à jour est un acte qui se voit dans un diff ». Test de sortie : `tests/e2e/WORKER-PINNED.json` porte le dépôt **et** un SHA complet ; un pin absent **refuse** au lieu de retomber sur `main`, même règle que `workerRepo` applique à `LOCUS_E2E_WORKER` ; une branche, un tag ou un SHA court sont refusés — les deux premiers se déplacent, le troisième devient équivoque, et les trois rendraient l'épinglage illusoire ; le workflow lit le fichier et **vérifie après coup** que la révision montée est celle du pin, sans quoi un `ref:` vide ferait silencieusement reprendre `main` |
...
| W20.j `[M]` **fait** | l'**idempotence du client** devient un fait du journal | `W20.h` a livré le chemin d'écriture et laissé cette garantie fausse là où elle sert : le registre des clés de §22.5 vit en mémoire vive, et un redémarrage — précisément ce qui coupe les connexions et déclenche les retentes — l'oublie. L'ADR 0029 décision 4 tranche la forme ; c'est une **migration**, donc son propre item, et `CLAUDE.md` demande qu'une migration ne se mélange pas à un changement de signature et à une reprise de concurrence dans le même commit. Test de sortie : une commande retentée après un redémarrage **ne s'exécute pas deux fois**, vérifié en reconstruisant depuis le journal ; le registre est une projection de plus à côté des quatre de §9.5, et il se rattrape comme elles ; l'enveloppe gagne **un** champ facultatif — la portée est déjà là, `workspace_id` et `actor.principal_id` — et un événement écrit avant la migration se relit **sans clé** plutôt qu'avec une clé vide, comme `W21.m` traite une écriture non classée ; rien n'expire, et la décision 5 dit pourquoi la durée de rétention est une valeur de politique et non une constante ; `#[serde(default)]` des deux côtés, sans quoi le rollback demanderait de réécrire le journal  **Fait.** L'enveloppe de §10.1 gagne `idempotency_key: Option<String>`, la colonne PostgreSQL est **nullable** et s'ajoute par un `alter table … add column if not exists` idempotent, et la lecture la prend en **fin** de liste — l'insérer au milieu aurait décalé tous les index suivants, et un décalage d'index se lit comme une donnée corrompue plutôt que comme une erreur de requête. La clé est apposée par la **transaction**, sur **chaque** événement de l'écriture et jamais par un producteur : la reconstruction lit le flux événement par événement sans savoir lesquels formaient une écriture, donc ne marquer que le premier ferait dépendre le registre de l'ordre de lecture. `Ledger` devient une projection reconstruite dans les **deux** constructeurs — n'en câbler qu'un serait le défaut qu'on trouve trois mois plus tard, et le second est celui que le binaire emploie quand un exploitant borne les écritures. **Une passe de mutants a démoli la première rédaction des tests** : `unwrap_or("")` sur une clé absente et `min` au lieu de `max` sur le rang survivaient tous les deux, parce que toutes les commandes des tests n'écrivaient qu'**un** événement — pour un seul, `max` et `min` sont le même — et parce qu'une clé vide est inatteignable par `submit`, `CommandEnvelope::mutating` la refusant. Les deux sont désormais tenus là où ils s'observent : un décideur qui écrit deux événements pour le rang, et des tests unitaires sur `Ledger::rebuild` pour l'absence. Six mutants posés, zéro survivant. **Ce qui reste ouvert et n'était pas dans l'ADR** : `Batch::Atomic` écrit une seule fois avec l'enveloppe de la **première** commande, donc le journal ne porte que sa clé et la reconstruction ne restaure qu'elle. Aucun appelant de production n'existe — `submit_batch` n'est appelé que par ses tests — et le registre en mémoire continue de les retenir toutes, donc rien ne régresse ; mais la garantie durable d'un lot atomique est celle de sa première commande, et c'est `W20.ac` |
| W20.ac `[R]` **bloqué — attend une décision de forme** | un lot **atomique** ne rend durable que l'idempotence de sa **première** commande | trouvé en livrant `W20.j`, et l'ADR 0029 décision 4 ne l'avait pas vu : elle dit « il ne manque que **la** clé choisie par le client », au singulier, ce qui est exact pour `submit` et silencieusement incomplet pour `Batch::Atomic`. Un lot atomique est **une** écriture, faite avec l'enveloppe de la première commande ; le journal ne porte donc que sa clé, et la reconstruction ne restaure qu'elle. **Rien ne régresse aujourd'hui** — le registre en mémoire retient toujours les `n`, et `submit_batch` n'a aucun appelant de production —, mais la garantie durable d'un lot atomique est celle de sa première commande, et personne ne s'en apercevrait avant le premier appelant réel. Les formes possibles s'excluent : porter les `n` clés sur l'écriture contredit « un seul champ facultatif » et demande un champ pluriel ; n'accepter qu'un lot dont les commandes partagent une clé change la sémantique de §22.5 ; écrire un événement par commande retire l'atomicité, qui est ce que le lot déclare. attend: décision |
