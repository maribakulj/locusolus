...
| W5.q `[R]` | **ce que `podman exec` dit quand il refuse** | le conteneur est **vivant** — `is_running` répond, et les sondes rendent « code générique », jamais « sandbox morte » — et `podman exec` refuse malgré tout pendant plus de six secondes. Ni cgroup transitoire (`W5.o`), ni sandbox morte (`W5.p`). Le harnais **jette** aujourd'hui ce que le runtime écrit sur son erreur, et c'est la seule chose qui reste à lire. Test de sortie : `stderr` du refus est conservé et rapporté à côté du code ; un refus qui ne dit rien est distingué d'un refus qui dit quelque chose, parce que les deux n'appellent pas la même suite |
