...
| W4.h `[R]` | la **surface** du broker : par où `locusd` lui parle | locusolus | découvert en écrivant `W22.c` — **aucun code du dépôt ne construit de client vers `locus-execd`, et le broker n'écoute rien.** L'audit du 2026-08-21 comptait trois maillons manquants ; c'est le quatrième, et il ne se voyait pas parce que les deux côtés sont cohérents séparément. Test de sortie : `locusd` atteint le broker par un transport nommé dans un ADR, et un test exerce l'aller-retour de bout en bout ; la quatrième frontière tient — `apps/locusd` n'acquiert aucune dépendance vers un SDK de runtime, et la garde le vérifie sur le nouveau chemin ; un broker injoignable se dit, et ne se confond pas avec un broker qui refuse |
