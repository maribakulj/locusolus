...
| # | Commit | Dépôt | Test de sortie |
|---|---|---|---|
| W22.a `[R]` | la garde de roadmap voit **toutes** les formes d'identifiant du plan, et les huit lignes invisibles reçoivent leur marqueur | locusolus | la garde reconnaît `W4.d.1` comme elle reconnaît `W4.d`, et le **déclare** : elle imprime le nombre de lignes reconnues, et une baisse de ce nombre est un échec — sans quoi une cécité future se lirait « rien à vérifier » ; la vérification en rouge se fait sur une ligne réellement présente et non sur une ligne inventée pour l'occasion ; les huit lignes (`W4.d.1` à `W4.d.4`, `W4.e.1`, `W4.f.1`, `W4.g.1`, `W4.g.2`) portent ensuite leur marqueur, chacune contre l'entrée de ledger qui l'atteste, ou disent ce qui manque ; le motif n'est **pas** élargi à des formes qu'aucune ligne n'emploie, décision 4 de l'ADR 0025 |
| W22.b `[R]` | la garde de roadmap : le couple marqueur ↔ entrée de ledger, dans les **deux** sens | locusolus | un item sans `**fait**` dont une entrée de ledger existe fait échouer la CI ; l'inverse aussi, et il est déjà tenu — les deux refus portent le même nom de famille et se lisent ensemble ; la garde ne lit pas la prose des items, seulement le couple ; une prose historique du ledger, qui décrit délibérément des états passés, ne la déclenche pas, et un test l'exerce sur une entrée ancienne |
| W22.c `[R]` | `main.rs` de `locus-execd` : le binaire ne nie plus ce que son crate exporte | locusolus | le binaire construit le driver, ou refuse en nommant ce qui manque **réellement** — un hôte, une configuration — et jamais un driver que son crate exporte ; la vérification en rouge est faite en niant une capacité réellement exportée, et non en éditant le message ; le refus, quand il a lieu, reste distinct d'un démarrage réussi par son code de sortie, comme `locusd` le fait déjà pour une projection en quarantaine |
| W22.d `[R]` | la garde de cohérence déclaration/symbole dans `tooling/` | locusolus | la garde cherche un **acte** — un motif de refus dans un point d'entrée dont le crate exporte le symbole déclaré manquant — et non un mot dans de la prose, ce qui serait la faute que `W4.c` a commise puis corrigée ; elle déclare le nombre de points d'entrée réellement examinés, et un décompte nul est un échec ; elle s'appuie sur le décompte déjà exposé par `check-boundaries.ts`, vérifié présent, plutôt que d'en reconstruire un ; un test l'exerce sur une prose historique légitime du ledger et vérifie qu'elle ne se déclenche pas |
| W22.e `[R]` | la sonde réelle de Canterel : `bubblewrapWorks` et `diskFreeMb` | canterel | `bubblewrapWorks` **démarre** `bwrap` comme le contrat de `capability-manifest.ts:33` l'exige, et un test de contrat exerce l'**adaptateur réel**, pas un `HostProbe` injecté ; `diskFreeMb` est mesuré ; une sonde qui ne peut pas conclure rend **l'absence** et non zéro, comme `W4.b` traite une sonde non exécutée — c'est le seul des quatre défauts qui porte sur un **niveau d'isolation**, et un `CapabilityManifest` erroné remonterait jusqu'à `place` de `W4.g`, qui choisirait un hôte sur une preuve qui n'en est pas une ; aucun fichier de `src/session/`, `agent/`, `permission/`, `provider/`, `tool/` n'est touché, tenu par `upstream.test.ts` |
| W22.f `[R]` | le commentaire d'inertie de `locus/index.ts` | canterel | la prose du point d'entrée décrit l'état **courant** ; l'historique va au ledger ; un test échoue si un identifiant d'item marqué livré dans `IMPLEMENTATION_LEDGER.md` est invoqué comme condition non satisfaite dans `src/locus/` ; l'inertie du worker n'est pas levée ici — elle est levée par `W2.20` —, seule la **raison** invoquée est rendue vraie, et confondre les deux ferait passer une correction de vérité pour une livraison |

**L'ordre importe pour les deux premiers.** `W22.a` répare l'instrument, `W22.b` lui ajoute une vérification que l'instrument réparé rend enfin exerçable sur les huit lignes. Les quatre suivants sont indépendants entre eux.
